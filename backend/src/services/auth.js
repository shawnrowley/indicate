...
class Auth {
  static async signup(email, password, options = {}, host) {
    ...
    if (user) {
      ...
      await UserDBApi.updatePassword(
        user.id,
        hashedPassword,
        options,
      );
  ...
  static async sendEmailAddressVerificationEmail(
    ...
    let link;
    try {
      const token = await UserDBApi.generateEmailVerificationToken(
        email,
      );
  ...
  static async sendPasswordResetEmail(email, type = 'register', host) {
    ...
    try {
      const token = await UserDBApi.generatePasswordResetToken(
        email,
      );
  ...
  static async verifyEmail(token, options = {}) {
    const user = await UserDBApi.findByEmailVerificationToken(
      token,
      options,
    );
    ...
    return UserDBApi.markEmailVerified(
      user.id,
      options,
    );
  }
  ...
  static async passwordReset(
    ...
    const user = await UserDBApi.findByPasswordResetToken(
      token,
      options,
    );
